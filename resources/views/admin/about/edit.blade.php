...
<div class="p-6 bg-gray-50 min-h-screen">
    ...
    <form action="{{ route('admin.about.update', $about->id) }}" method="POST" class="bg-white p-6 rounded shadow space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label>Title (EN)</label>
            <input type="text" name="title_en" value="{{ old('title_en', $about->title_en) }}" class="mt-2 block w-full border p-2 rounded">
        </div>

        <div>
            <label>Title (TA)</label>
            <input type="text" name="title_ta" value="{{ old('title_ta', $about->title_ta) }}" class="mt-2 block w-full border p-2 rounded">
        </div>

        <div>
            <label>Description (EN)</label>
            <textarea id="desc_en" name="description_en" class="mt-2 block w-full border p-2 rounded">{{ old('description_en', $about->description_en) }}</textarea>
        </div>

        <div>
            <label>Description (TA)</label>
            <textarea id="desc_ta" name="description_ta" class="mt-2 block w-full border p-2 rounded">{{ old('description_ta', $about->description_ta) }}</textarea>
        </div>

        <div class="flex items-center space-x-2 mt-2">
            <label class="font-medium text-gray-700">Publish</label>
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" value="1" name="is_public" class="sr-only peer" {{ $about->is_public ? 'checked' : '' }}>
